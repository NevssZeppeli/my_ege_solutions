...
file = open('27-63b.txt')
N = int(file.readline())
M = 9  # Интересует делимость на 9.
K = 4  # Сумма должна состять ровно из 4 чисел.
...
for i in range(N):
    ...
    for n in range(1, K+1):
        for m in range(M):
            ...
            s_new[n][m] = min(variants) if variants else None
    s = s_new
print(min(s[4][m] for m in range(1, M)))
